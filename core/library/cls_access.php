...
class cls_access
{
     ...
     *
     */
    function __construct($pool_name) // admin
    {
   ...
   /*
    * 用工厂方法创建对象实例
    * @parem $poolname admin
    * @parem $cp_url ?ct=index&ac=login
    *
    * @return object
    */
    ...
    */
     ...
     * 删除用户缓存
     */
     public static function del_cache( $uid )
     {
         return cache::del(self::$info_cache, $uid);
     }
     ...
     */
      ...
      */
      public function test_purview($mod, $action, $backtype='1')
      {
           ...
           //检测开放控制器和事件
           if( !empty(self::$cfg_groups['pools'][$this->pool_name]['public']) && 
               ( self::$cfg_groups['pools'][$this->pool_name]['public']=='*' || in_array($action, $public_mod) || in_array('*',     $public_mod) )
             )
           {
               $rs = 1;
           }
    ...
    public function get_cookie($key, $encode=true)
    {
         ...
         else
         {
              if($encode)
              {
                 ...
                 if( !isset($_COOKIE[$key.'_ymkv']) ) return '';
                 else return ($_COOKIE[$key.'_ymkv'] != $epwd ) ? '' : $_COOKIE[$key];
              }
              else
              {
