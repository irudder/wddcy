...
class cls_lurd
{
    ...
    */
	public function insert($key_value_array, $tablename='', $use_default=false,$insert_id = false)
	{
		...
		$rs = db::query( $this->last_sql, true );
                if($insert_id) return db::insert_id();
		return $rs;
	}
	...
	public function get_datas( $pageno=1, $listfield = '*', $orderquery ='', $wherequery='', $tablename='', $count_row=true )
	{
		...
		if($islink)
		{
			...
			foreach($this->link_tables as $k=>$link_table)
			{
				...
				{
					if($v['table'] != $link_tablename) continue;
    ...
    * @return string
    */
	public function get_pagination( $url = '', $style = "ylmf-page", $show_total_infos=false )
	{
		...
		if (empty($config) or $config['count_page'] < 2)
		{
			if( $show_total_infos )
			{
		        return "<span>共 {$config['count_number']} 记录</span>\n";
		    } else {
			    return false;
		    }
		}
    ...
    */
	public function analyse_table()
	{
		...
		$tb_infos = false;
		//$tb_infos = cache::get('lurd_', $infos_file);
		...
		for($i=1; $i < count($flines)-1; $i++ )
		{
			...
			if( !in_array($this->fields[$lines[0]]['type'], $this->text_types)
			&& !in_array($this->fields[$lines[0]]['type'], $this->bin_types) )
			{
		...
		$tb_infos['primary_key'] = $this->primary_key;
		cache::set('lurd_', $infos_file, $tb_infos, 86400);
