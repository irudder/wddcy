...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
        ...
        preg_match("@<return_code><\!\[CDATA\[(.*?)\]\]></return_code>@", $response, $out);
        if (empty($out[1]) || $out[1] != "SUCCESS") 
        {
            preg_match("@<return_msg><\!\[CDATA\[(.*?)\]\]></return_msg>@", $response, $out);
            $msg = empty($out[1]) ? '' : $out[1];
	...
	{
		//获取通知的数据
		$xml = $xml ? $xml : $GLOBALS['HTTP_RAW_POST_DATA'];
		//如果返回成功则验证签名
		try {
