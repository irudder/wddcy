...
/* $Id: Smarty_Compiler.class.php 436 2009-12-24 08:54:44Z lx $ */
...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
        ...
        /*
        设定了rewrite重写后，对内容里的链接按指定规则进行转换
        if( isset($GLOBALS['config']['use_rewrite']) && $GLOBALS['config']['use_rewrite']==true )
        {
            cls_rewrite::convert_html($source_content, 0);
        }
        */
    ...
    function _compile_tag($template_tag)
    {
        ...
        switch ($tag_command) {
            ...
            default:
                $first = substr($tag_command, 0, 6);
                if( preg_match('/^(cms|\/cms|lurd|\/lurd|my|\/my)_/', $first) && $this->_compile_myblock_tag($tag_command, $tag_args, $output) ) {
                    return $output;
    ...
    function _compile_myblock_tag($tag_command, $tag_args, &$output)
    {
        ...
        if ($found)
        {
            if ($have_function)
            {
                ...
                $keyname = empty($attrs['key']) ? 'key' : $attrs['key'];
                $keyname = preg_replace('/["\']/', '', $keyname);
                
                $output .= "foreach( \${$tag_command} as \$this->_tpl_vars['{$keyname}']=>\$this->_tpl_vars['".$fieldname."'] )\n{\n";
                $output .= '?>';
            } else {
                $this->_syntax_error($message, E_USER_WARNING, __FILE__, __LINE__);
            }
