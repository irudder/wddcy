...
class util
{
    ...
    public static function get_cururl()
    {
        ...
        {
            $scriptName = "http://".$_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
            $nowurl = $scriptName;
        }
        else
        {
            $scriptName = $_SERVER["PHP_SELF"];
            $nowurl = "http://".$_SERVER["SERVER_NAME"].empty($_SERVER["QUERY_STRING"]) ? $scriptName : $scriptName . "?" . $_SERVER["QUERY_STRING"];
        }
        return $nowurl;
    }
     ...
     * @return string
     */
    public static function xml_to_array($contents, $get_attributes = 0)
    {
    ...
    public static function compress_html($string )
    {
        ...
        $string = '';//[phpdo.net]修改压缩html : 清除换行符,清除制表符,去掉注释标记
        foreach ( $chunks as $c )
        {
            if ( strpos( $c, '<pre' ) !== 0 )
            {
                //[phpdo.net] remove new lines & tabs
                $c = preg_replace( '/[\\n\\r\\t]+/', ' ', $c );
                // [phpdo.net] remove extra whitespace
                $c = preg_replace( '/\\s{2,}/', ' ', $c );
                // [phpdo.net] remove inter-tag whitespace
                $c = preg_replace( '/>\\s</', '><', $c );
                // [phpdo.net] remove CSS & JS comments
                $c = preg_replace( '/\\/\\*.*?\\*\\//i', '', $c );
            }
            $string .= $c;
        }
    ...
    public static function price_format($price, $change_format = true)
    {
        if($price == '')
        {
            $price = 0;
        ...
        {
            $PRICE_FORMAT = 4;
            //switch (C('PRICE_FORMAT'))//此处预留日后价格格式化类型选择，暂时 4：四舍五入，保留 1 位
            switch ($PRICE_FORMAT)//此处预留日后价格格式化类型选择，暂时 4：四舍五入，保留 1 位
            {
            case 0:
    ...
    public static function make_order_id()
    {
        // 生成订单号
        $rand = "";
        for ($i = 0; $i < 7; $i ++)
    ...
    public static function make_ec_salt()
    {
        $ec_salt = '';
        for($i=1;$i<5;$i++) 
        {
    ...
    public static function ajax_return($return)
    {
