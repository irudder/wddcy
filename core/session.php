...
if( SESSION_TYPE == 'memcache' )
{
    ...
    ini_set('session.save_path', $GLOBALS['config']['cache']['memcache']['host'][0]);
    session_set_cookie_params(0, "/", ".dcyfood.com");
    // 不用注册 init、close、read、write、destroy、gc方法，memcache自带了，当然也可以注册了，然后做分布式到不同的memcache去
}
else
{
    //接口参数
